...
/obj/effect/landmark/zcontroller/proc/calc(var/list/L)
	...
	while(L.len)
		...
		if(down && (istype(T, /turf/space) || istype(T, /turf/simulated/floor/open) || istype(T, /turf/simulated/floor/plating/catwalk) || istype(T, /obj/structure/catwalk)))
			...
			if(below)
				...
				T.overlays += image('icons/turf/floors.dmi', icon_state = "black_open", layer = TURF_LAYER+0.4)
				T.z_overlays += image('icons/turf/floors.dmi', icon_state = "black_open", layer = TURF_LAYER+0.4)
